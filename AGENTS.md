...
- `pnpm check`: run Biome across the repo. This is the only dedicated lint/format command.
...
- Biome enforces double quotes, import organization, and Tailwind class sorting, including classes passed through `clsx`, `cva`, and `cn`.
...
- `getBaseUrl()` only checks `NEXT_PUBLIC_VERCEL_URL`; if metadata or OG behavior depends on absolute URLs, that is the env var the app already expects.
